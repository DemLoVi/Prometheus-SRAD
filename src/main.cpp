...
void get_GPS_data() {
  ...
  // модуль работает по UART, у нас на плате есть апаратно, Serial1 Serial2 соответсенно
  // Serial2 я заберу под отправку СМС и работу с тем модулем, тебе остаётся использовать Serial1
  // Прошу обратить на это внимание, потому что в ппримерах всегда поднимают програмную реализацию через SoftSerial
  // Мы можем не использовать его а использовать просто Serial1
}
